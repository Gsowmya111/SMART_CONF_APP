...
public class A_rc extends View {

	boolean isCanvasDrawnAlready=false;
	...
	private final int paramArrowPointerSize = 6;//4; // size of the arrow pointer; set to 0 to hide the pointer
	...
	public A_rc(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public A_rc(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public A_rc(Context context) {
		super(context);
		init();
	}
	...
	protected void onSizeChanged(int width, int height, int oldw, int oldh) {
		...
		Log.d("TAG","canvas drawn  called :"+isCanvasDrawnAlready);
		if(!isCanvasDrawnAlready){

			isCanvasDrawnAlready=true;
